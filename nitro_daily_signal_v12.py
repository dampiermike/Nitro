#!/usr/bin/env python3
"""
Dampier Nitro++ v12 Daily Signal Generator
Runs after market close. Reads the latest history CSVs, replays the full
Nitro++ v12 engine through today, then emails trade instructions.

v12 changes vs v10:
  - TQQQ hard stop tightened: −6% (0.94×) from −5% (0.95×)
  - MTI exit: while in TQQQ, if MTI < 0.75 and not entry bar → exit at NEXT open
  - v12 C/Up SQQQ block: Priority 4 SQQQ entries blocked during C/Up regime
  - Pyramid overlay: once TQQQ trade up ≥20%, position sized at 1.25× for dret
  - TQQQ stitch: explicit 2010-03-31 cutoff (not dynamic real_start)
  - Updated LIVE_START to 2026-04-07

Usage:
    python3 nitro_daily_signal_v12.py
"""
...
def load_data():
    ...
    # Stitch TQQQ — v12: explicit 2010-03-31 cutoff
    cutoff = pd.Timestamp('2010-03-31')
    tqqq_stitched = pd.concat([syn[syn['Date'] < cutoff], tqqq_real[tqqq_real['Date'] >= cutoff]],
    ...
    # tqqq_atr: simple 10-bar ATR%, manual numpy loop on FULL stitched series
...
def run_engine(df):
    ...
    # Health factor array (for equity display — does NOT affect trade logic)
    ...
    for i in range(N):
        ...
        if in_trade:
            if inst == 'TQQQ':
                ...
                # v12 pyramid: set once when trade is up ≥20%; never reset mid-trade
...
def get_signal(df, trades, final_state):
    ...
    pyramid_active  = final_state['pyramid_active']
    ...
    if in_trade:
        ...
        if inst == 'TQQQ':
            ...
            if pyramid_active:
                notes.append("Pyramid ACTIVE: trade up ≥20% — position effectively sized at 1.25×")
            elif unrealized >= 0:
                pct_to_pyramid = 20.0 - unrealized
                if pct_to_pyramid > 0:
                    notes.append(f"Pyramid threshold: {pct_to_pyramid:.1f}% more gain to activate 1.25× sizing")
            if tp_level:
                notes.append(f"TP target:  TQQQ closes ≥ ${tp_level:.2f} (+50% from entry open)")
            if tqqq_stop:
                notes.append(f"Hard stop:  TQQQ closes ≤ ${tqqq_stop:.2f} (-6% from entry open) → exit at NEXT open")
            notes.append(f"MTI exit:   if MTI < 0.75 (currently {mti:.3f}) → exit at NEXT open")
            if not cdn_active:
        ...
        # v12: SQQQ signals blocked during C/Up
        ...
        if cup_signal:
            ...
            if regime_i == 'C/Up':
                ...
                reasons.append("  Note: SQQQ (DEW Sell) entries are blocked in C/Up regime (v12)")
            else:
