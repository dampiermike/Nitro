...
# ── File paths ──────────────────────────────────────────────────────────────
QQQ_FILE     = '/Users/mikedampier/Downloads/qqq from VV.csv'
TQQQ_FILE    = '/Users/mikedampier/Downloads/TQQQ OHLC (1).csv'
SYN_FILE     = '/Users/mikedampier/Downloads/synthetic_TQQQ_OHLC_1999_2010.csv'
VV_FILE      = '/Users/mikedampier/Downloads/VectorVest Views w3Place Precision.csv'
VIX_FILE     = '/Users/mikedampier/Downloads/^VIX_historical_data.csv'
SQQQ_FILE    = '/Users/mikedampier/Downloads/SQQQ_historical_data.csv'
...
# ── 1a. QQQ ─────────────────────────────────────────────────────────────────
qqq_raw = pd.read_csv(QQQ_FILE, dtype=str)
qqq_raw = qqq_raw[~qqq_raw['Date'].isin(['Date', 'Time'])].copy()
qqq_raw['Date'] = pd.to_datetime(qqq_raw['Date'])
qqq_raw = qqq_raw.sort_values('Date').reset_index(drop=True)

for col in ['Open', 'High', 'Low', 'Close']:
    qqq_raw[col] = pd.to_numeric(qqq_raw[col])

# Compute QQQ return columns (MUST come from OHLC file, not VVC-Price)
...
# PSQ columns (1× inverse QQQ exposure; expense ratio only in c2c)
qqq_raw['o2c_PSQ']  = -1 * qqq_raw['o2c_QQQ']
qqq_raw['pc2o_PSQ'] = -1 * qqq_raw['pc2o_QQQ']
qqq_raw['c2c_PSQ']  = -1 * qqq_raw['c2c_QQQ'] - 0.0095 / 252

# Full QQQ series saved for DEW warm-up (Step 2)
qqq_full = qqq_raw.copy()

# Filter to backtest window for merge
...
# RT_v6 is in column 'RT_v6'

# ── 1c. Real TQQQ (2010+) ───────────────────────────────────────────────────
tqqq_raw = pd.read_csv(TQQQ_FILE, dtype=str)
tqqq_raw = tqqq_raw[~tqqq_raw['Date'].isin(['Date', 'Time'])].copy()
tqqq_raw['Date'] = pd.to_datetime(tqqq_raw['Date'])
tqqq_raw = tqqq_raw.sort_values('Date').reset_index(drop=True)
for col in ['Open', 'High', 'Low', 'Close', 'RT']:
    tqqq_raw[col] = pd.to_numeric(tqqq_raw[col])

# ── 1d. Stitch TQQQ ─────────────────────────────────────────────────────────
# Synthetic covers up to 2010-03-30; real starts 2010-02-11.
# Use synthetic for dates strictly before real TQQQ launch (2010-02-11),
# then real from 2010-02-11 onward.
real_start = tqqq_raw['Date'].min()

syn_part  = syn[syn['Date'] < real_start].copy()
real_part = tqqq_raw.copy()

# Build unified TQQQ dataframe
tqqq_syn = pd.DataFrame({
    'Date':      syn_part['Date'],
    'tqqq_open': syn_part['Open'],
    'tqqq_high': syn_part['High'],
    'tqqq_low':  syn_part['Low'],
    'tqqq_close':syn_part['Close'],
    'tqqq_rt':   syn_part['RT_v6'],
})

tqqq_real = pd.DataFrame({
    'Date':      real_part['Date'],
    'tqqq_open': real_part['Open'],
    'tqqq_high': real_part['High'],
    'tqqq_low':  real_part['Low'],
    'tqqq_close':real_part['Close'],
    'tqqq_rt':   real_part['RT'],
})

tqqq_stitched = pd.concat([tqqq_syn, tqqq_real], ignore_index=True)
tqqq_stitched = tqqq_stitched.sort_values('Date').reset_index(drop=True)

# TQQQ return columns (using close as adjusted close)
...
# tqqq_atr: simple 10-bar ATR% on FULL stitched series (including pre-2000)
tr_t = pd.concat([
...
tqqq_stitched['tqqq_atr'] = (
    tr_t.rolling(10).mean() / tqqq_stitched['tqqq_close'] * 100
)
...
print(f"\ndf.shape[0] = {df.shape[0]}  (target: 6588)")
...
print(f"DEW blank      = {blank}  (target: 6199)")
...
print(f"Final:  ${final_3b:,.0f}  (target: $414,000)")
print(f"CAGR:   {cagr_3b*100:.2f}%  (target: 5.58%)")
print(f"Sharpe: {sharpe_3b:.4f}  (target: 0.3468)")
...
print(f"Trades: {len(trades_3c)}  (target: 98)")
print(f"Final:  ${final_3c:,.0f}  (target: $104,300)")
print(f"CAGR:   {cagr_3c*100:.2f}%  (target: 0.16%)")
print(f"Sharpe: {sharpe_3c:.4f}  (target: 0.0643)")
...
print(f"CAGR:   {cagr_3d*100:.4f}%  (target: 1.4028%)")
print(f"Sharpe: {sharpe_3d:.4f}  (target: 0.2122)")
...
print(f"CAGR:       {cagr_3e*100:.4f}%  (target: 4.7304%)")
print(f"Sharpe:     {sharpe_3e:.4f}  (target: 0.6394)")
...
chains_7   = len(set(t['entry'] for t in trades_7
                     if sum(1 for t2 in trades_7 if t2['entry']==t['entry'])>1))
...
print(f"CAGR:    {cagr_7*100:.2f}%  (target: 34.56%)")
print(f"Sharpe:  {sharpe_7:.4f}  (target: 1.3137)")
